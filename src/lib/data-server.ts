// ================================================================
// data-server.ts — Versión server-safe del data layer
// Usa createServerClient en vez de createBrowserClient
// ================================================================
...
export async function generarAlertas(metodo: MetodoProyeccion = "mediana"): Promise<Alerta[]> {
  const [ingredientes, historico, inventario, ordenes] = await Promise.all([
    fetchIngredientes(), fetchHistorico(), fetchInventario(), fetchOrdenes(),
  ]);
  ...
  for (const suc of sucursales) {
    for (const [ingId, ing] of mapaCat) {
      const histSuc = historico.filter((h) => h.sucursal === suc && h.ingrediente_id === ingId).map((h) => h.consumo_unidad_base);
      ...
      if (ordenItem && histSuc.length === 0) {
        alertas.push({
          sucursal: suc, ingrediente_id: ingId, nombre: ingId, proveedor: "Desconocido", formato: "?", unidad_base_por_formato: 1, unidad_base: "?",
          consumo_proyectado: 0, stock_actual: 0, necesidad_real: 0, formatos_necesarios: 0,
          formatos_pedidos: ordenItem.cantidad_formatos, diferencia_formatos: ordenItem.cantidad_formatos, tipo: "sin_datos",
      ...
      alertas.push({
        sucursal: suc, ingrediente_id: ingId, nombre: ing.nombre, proveedor: ing.proveedor, formato: ing.formato_compra,
        unidad_base_por_formato: ing.unidad_base_por_formato, unidad_base: ing.unidad_base,
        consumo_proyectado: proyectado, stock_actual: invSuc, necesidad_real: necesidad_unidades,
        formatos_necesarios, formatos_pedidos: formatosPedidos, diferencia_formatos: diff, tipo, mensaje,
      });
    }
  }
  ...
  for (const o of ordenes) {
    if (!mapaCat.has(o.ingrediente_id)) {
      alertas.push({
        sucursal: o.sucursal, ingrediente_id: o.ingrediente_id, nombre: o.ingrediente_id, proveedor: "Desconocido",
        formato: "?", unidad_base_por_formato: 1, unidad_base: "?",
        consumo_proyectado: 0, stock_actual: 0, necesidad_real: 0, formatos_necesarios: 0,
        formatos_pedidos: o.cantidad_formatos, diferencia_formatos: o.cantidad_formatos, tipo: "sin_datos",
        mensaje: `⚠️ "${o.ingrediente_id}" no existe en catálogo.`,
      });
    }
  }
...
export function agruparPorProveedor(alertas: Alerta[]): Record<string, Alerta[]> {
  ...
  for (const a of alertas) { if (!g[a.proveedor]) g[a.proveedor] = []; g[a.proveedor].push(a); }
  return g;
}
